...
int main(int argc, char *argv[]) {
  ...
  if (command == "init") {
    ...
    const auto blob_file = blob_hash.substr(2);

    const auto blob_path =
        std::filesystem::path(".git") / "objects" / blob_dir / blob_file;
    ...
    const auto blob_data = std::string(std::istreambuf_iterator<char>(file),
    ...
    while (true) {
      auto len = buf.size();
      if (auto res =
              uncompress((uint8_t *)buf.data(), &len,
                         (const uint8_t *)blob_data.data(), blob_data.size());
          res == Z_BUF_ERROR) {
        buf.resize(buf.size() * 2);
      } else if (res != Z_OK) {
        std::cerr << "Failed to uncompress Zlib";
        return EXIT_FAILURE;
      } else {
        buf.resize(len);
        break;
